...
n_relations  = dataset[0].edge_index.shape[1]  
# number of edges
...
for epoch in range(n_epoch):
    ...
    loss = F.nll_loss(predicted[data.test_mask], data.y[data.test_mask])  
    ...
    if(epoch%5==0):
        ...
        train_correct = float (pred[data.test_mask].eq(data.y[data.test_mask]).sum().item())
        correct = float (pred[data.train_mask].eq(data.y[data.train_mask]).sum().item())
        acc = correct / data.train_mask.double().sum().item()
        train_acc = train_correct / data.test_mask.double().sum().item()
